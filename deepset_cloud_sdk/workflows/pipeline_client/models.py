...
class PipelineInputs(BaseModel):
    ...
    query: List[str] = Field(
        ...
        description=PipelineServiceDocs.PIPELINE_QUERY_INPUTS_DESCRIPTION
    )
    filters: List[str] = Field(
        default_factory=list,
        description=PipelineServiceDocs.PIPELINE_FILTERS_INPUTS_DESCRIPTION
    )
    files: List[str] = Field(
        default_factory=list,
        description=PipelineServiceDocs.INDEX_FILES_INPUTS_DESCRIPTION
    )
...
class PublishConfig(BaseModel):
    ...
    name: str = Field(
        ...,
        description=PipelineServiceDocs.PUBLISH_CONFIG_NAME_DESCRIPTION,
        min_length=1
    )
    pipeline_type: PipelineType = Field(..., description=PipelineServiceDocs.PUBLISH_CONFIG_PIPELINE_TYPE_DESCRIPTION)
    inputs: PipelineInputs = Field(default_factory=PipelineInputs, description=PipelineServiceDocs.PUBLISH_CONFIG_INPUTS_DESCRIPTION)
    outputs: PipelineOutputs = Field(default_factory=PipelineOutputs, description="Output configuration for the pipeline")
